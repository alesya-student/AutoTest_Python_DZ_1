...
@allure.story('Автотест на заполнение формы.')
@allure.title('Тест на проверку заполненных полей.')
...
@allure.epic("Форма регистрации.")
@allure.story('Автотест на заполнение формы.')
@allure.title('Тест на соответствие id поля и цвета поля.')
@allure.description('Возможно проверить любое поле,'
                    'подставляя id и цвет на соответствие.')
...
def test_color_field():
    ...
    assert attribute_color == 'rgba(0, 0, 0, 1)'
...
@allure.epic("Форма регистрации.")
@allure.story('Автотест на заполнение формы.')
@allure.title('Тест на проверку заполненных/не заполненных полей')
@allure.description('проверка класса (class*):'
                    '*alert-danger - не заполненное поле;'
                    '*alert-success - заполненное поле')
@allure.severity(allure.severity_level.MINOR)
def test_class():
    browser = webdriver.Chrome(service=ChromeService(ChromeDriverManager().
                                                     install()))
